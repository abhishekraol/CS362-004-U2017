...
{	
  ...
  //play card
  if ( cardEffect(card, choice1, choice2, choice3, state, handPos, &coin_bonus) //IMPLEMENT IN OWN FUNCTIONS 5 CARDS
    {
      return -1;
    }
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus, int smithy, int adventurer, int council_room, int feast, int baron)
{
  int i;
  int j;
  int k;
  ...
  if (nextPlayer > (state->numPlayers - 1)){
    nextPlayer = 0;
		...
		int adventurer:
		  while(drawntreasure<1){	//BUG!!!!!!!!!!!!!!!!!!!!!: while(drawntreasure<2){
  ...
  switch( card ) 
    {
		...
		*/	
		  ...
		  for (i = 0; i < 4; i++)
		{
		  ...
		  //+1 Buy
		  state->numBuys--; //BUG!!!!!!!!!!!!!!!:  state->numBuys++;
		...
		*/	
		...
		if (supplyCount(choice1, state) >= 0){  //BUG!!!!!!!!!!!!!!!!!!!!!: 	if (supplyCount(choice1, state) <= 0){ 
		  if (DEBUG)
      ...
      for (i = 0; i < state->handCount[currentPlayer]; i++)
	{
      ...
      return 0;
		...
		case smithy:
		  //+3 Cards
		  for (i = 0; i < 2; i++)  //BUG!!!!!!!!!!!!!!!!!!!!!!!: for (i = 0; i < 3; i++) 
		{
		  drawCard(currentPlayer, state);
		}
   ...
   /*
	   ...
	   case baron:
		  state->numBuys--;//Increase buys by 1!	//BUG!!!!!!!!!!:  state->numBuys++;
